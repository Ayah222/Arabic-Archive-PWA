...
const DarkAurora = () => (
  <div className="fixed inset-0 overflow-hidden pointer-events-none" style={{ zIndex: 0 }}>
    <div style={{ position: "absolute", width: 900, height: 750, borderRadius: "50%",
      background: "radial-gradient(ellipse, rgba(0,240,255,0.18) 0%, rgba(0,240,255,0.06) 40%, transparent 70%)",
      filter: "blur(90px)", top: -320, right: -120 }} />
    <div style={{ position: "absolute", width: 750, height: 600,
      background: "radial-gradient(ellipse, rgba(112,0,255,0.20) 0%, rgba(112,0,255,0.06) 45%, transparent 72%)",
      filter: "blur(110px)", top: -80, left: "12%" }} />
    <div style={{ position: "absolute", width: 620, height: 520,
      background: "radial-gradient(ellipse, rgba(255,0,128,0.14) 0%, rgba(112,0,255,0.04) 50%, transparent 74%)",
      filter: "blur(100px)", bottom: -200, left: -60 }} />
    <div style={{ position: "absolute", width: 400, height: 320,
      background: "radial-gradient(ellipse, rgba(0,240,255,0.09) 0%, transparent 70%)",
      filter: "blur(80px)", top: "48%", right: "5%" }} />
  </div>
);

const LightAurora = () => (
  <div className="fixed inset-0 overflow-hidden pointer-events-none" style={{ zIndex: 0 }}>
    <div style={{ position: "absolute", width: 900, height: 700,
      background: "radial-gradient(ellipse, rgba(99,102,241,0.14) 0%, rgba(99,102,241,0.04) 50%, transparent 72%)",
      filter: "blur(80px)", top: -200, right: -100 }} />
    <div style={{ position: "absolute", width: 700, height: 550,
      background: "radial-gradient(ellipse, rgba(168,85,247,0.12) 0%, rgba(168,85,247,0.03) 55%, transparent 74%)",
      filter: "blur(90px)", top: -100, left: "20%" }} />
    <div style={{ position: "absolute", width: 600, height: 450,
      background: "radial-gradient(ellipse, rgba(6,182,212,0.10) 0%, transparent 70%)",
      filter: "blur(100px)", bottom: -140, left: -60 }} />
    <div style={{ position: "absolute", width: 500, height: 380,
      background: "radial-gradient(ellipse, rgba(236,72,153,0.09) 0%, transparent 72%)",
      filter: "blur(90px)", bottom: -60, right: -40 }} />
  </div>
);
...
export default function MainLayout({ children }: MainLayoutProps) {
  ...
  const activeStyle: React.CSSProperties = isDark
    ? { background: "rgba(0,240,255,0.06)", border: "1px solid rgba(0,240,255,0.70)",
        boxShadow: "0 0 0 1px rgba(0,240,255,0.12), 0 0 20px rgba(0,240,255,0.28), inset 0 1px 1px rgba(0,240,255,0.10)",
        color: "#00f0ff" }
    : { background: "rgba(99,102,241,0.10)", border: "1px solid rgba(99,102,241,0.55)",
    ...
    <div className="min-h-screen text-foreground flex rtl relative" dir="rtl">
      ...
      >
        ...
        <div className="p-4 flex items-center gap-3 min-h-[68px] relative overflow-hidden"
          ...
          {isDark && <div style={{ position: "absolute", top: -10, right: -10, width: 90, height: 90,
            borderRadius: "50%", background: "rgba(0,240,255,0.08)", filter: "blur(28px)", pointerEvents: "none" }} />}

          <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 relative"
            style={{
              background: isDark ? "linear-gradient(135deg, #00f0ff 0%, #7000ff 100%)" : "linear-gradient(135deg, #6366f1 0%, #a855f7 100%)",
              boxShadow: isDark ? "inset 0 1px 1px rgba(255,255,255,0.25), 0 0 24px rgba(0,240,255,0.50), 0 0 50px rgba(0,240,255,0.18)" : "0 2px 14px rgba(99,102,241,0.42), inset 0 1px 1px rgba(255,255,255,0.30)",
          ...
          <div className="flex-1 min-w-0">
            <h1 className="font-bold text-sm leading-tight whitespace-nowrap overflow-hidden"
              style={{ color: isDark ? "#ffffff" : "#1e1b4b" }}>أرشيف ذكي</h1>
            <p className="text-[10px] mt-0.5 font-medium"
              style={{ color: isDark ? "rgba(0,240,255,0.55)" : "rgba(99,102,241,0.70)" }}>إدارة المشاريع</p>
          </div>

          <button className="md:hidden p-1 rounded-lg"
              ...
              <Link key={to} to={to} onClick={() => setSidebarOpen(false)}
                ...
                {active && (
                  <span className="text-xs font-bold relative z-10"
                    style={{ color: isDark ? "rgba(0,240,255,0.70)" : "rgba(99,102,241,0.70)" }}>+</span>
                )}
                <Icon className={`w-5 h-5 shrink-0 relative z-10 ${active ? "" : "opacity-50 group-hover:opacity-80 transition-opacity"}`} />
                ...
                {to === "/notifications" && unreadCount > 0 && (
                  <span className="text-xs font-black px-1.5 py-0.5 rounded-full shrink-0 relative z-10"
                    style={{ background: isDark ? "rgba(255,0,128,0.14)" : "rgba(236,72,153,0.10)",
      ...
      <div className="flex-1 flex flex-col min-w-0 relative" style={{ zIndex: 1 }}>
        ...
        <header className="h-14 md:h-16 flex items-center justify-between px-3 md:px-5 sticky top-0 overflow-hidden"
          ...
          <div className="flex items-center gap-2 md:gap-3">
            <button className="rounded-xl w-9 h-9 md:w-10 md:h-10 flex items-center justify-center"
              style={{ color: isDark ? "rgba(255,255,255,0.70)" : "#6b7280",
            ...
            <h2 className="font-bold text-sm md:text-base truncate"
              style={{ color: isDark ? "#ffffff" : "#1e1b4b" }}>{currentLabel}</h2>
          </div>

          <div className="flex items-center gap-1">
            {/* Bell */}
            <div className="relative" ref={bellRef}>
              <button ref={bellButtonRef}
                className="relative rounded-xl w-9 h-9 md:w-10 md:h-10 flex items-center justify-center"
                style={{ color: isDark ? "rgba(255,255,255,0.70)" : "#6b7280",
                  background: isDark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.04)",
                  border: isDark ? "1px solid rgba(255,255,255,0.06)" : "1px solid rgba(0,0,0,0.06)" }}
                ...
                {unreadCount > 0 && (
                  <span className="absolute top-0.5 right-0.5 min-w-[17px] h-[17px] px-[3px] text-[10px] font-black leading-none flex items-center justify-center rounded-full"
                    style={{ background: isDark ? "rgba(255,0,128,0.90)" : "rgba(220,38,100,0.88)", color: "#fff",
            ...
            {bellOpen && bellRect && (
              <div ref={dropdownRef}
                style={{ position: "fixed", top: bellRect.bottom + 8, left: bellRect.left,
                  width: 340, zIndex: 9999, borderRadius: 16, overflow: "hidden",
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5">
                            <p className="text-xs font-bold truncate"
                              style={{ color: n.read ? (isDark ? "rgba(255,255,255,0.38)" : "#9ca3af") : (isDark ? "rgba(255,255,255,0.90)" : "#1e1b4b") }}>
                              {n.title}
                            </p>
                            {!n.read && (
                          ...
                          <p className="text-[11px] mt-0.5 truncate"
                            style={{ color: isDark ? "rgba(255,255,255,0.40)" : "#6b7280" }}>{n.message}</p>
                        </div>
                      </div>
                    );
                  })}
            ...
            <button onClick={() => setIsDark(d => !d)}
              className="rounded-xl w-9 h-9 md:w-10 md:h-10 flex items-center justify-center"
              style={{ color: isDark ? "rgba(255,255,255,0.70)" : "#6b7280",
                background: isDark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.04)",
                border: isDark ? "1px solid rgba(255,255,255,0.06)" : "1px solid rgba(0,0,0,0.06)" }}>
